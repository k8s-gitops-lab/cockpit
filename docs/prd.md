...
Ce dépôt porte la vue globale d'un POC de chaîne CI/CD relativement complète,
autohébergée sur un cluster Kubernetes local. Tous les projets GitLab sont
...
fichiers `argocd/apps/*.yaml`, `ApplicationSet` ArgoCD, seed GitLab
généralisé et toolbox `toolbox`) est détaillé dans la spec technique.
